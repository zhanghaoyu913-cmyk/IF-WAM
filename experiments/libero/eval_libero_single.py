...
def _load_cached_text_context(prompt: str, cfg: DictConfig) -> tuple[torch.Tensor, torch.Tensor] | None:
    cache_dir = cfg.EVALUATION.get("text_embedding_cache_dir")
    if cache_dir is None:
        return None
    context_len = int(cfg.data.train.context_len)
    hashed = hashlib.sha256(prompt.encode("utf-8")).hexdigest()
    cache_path = Path(str(cache_dir)) / f"{hashed}.t5_len{context_len}.wan22ti2v5b.pt"
    payload = torch.load(cache_path, map_location="cpu", weights_only=True)
    context = payload["context"]
    context_mask = payload["mask"].bool()
    if context.shape != (context_len, 4096) or context_mask.shape != (context_len,):
        raise ValueError(f"Invalid cached text embedding shapes in {cache_path}: {context.shape}, {context_mask.shape}")
...
def _predict_action_chunk(
    ...
    *,
    ...
    cached_context = _load_cached_text_context(prompt, cfg)
    text_condition = ({"prompt": prompt} if cached_context is None else {"prompt": None, "context": cached_context[0], "context_mask": cached_context[1]})
