...
run_libero_eval() {
    ...
    launch_task_on_pane() {
        ...
        tmux send-keys -t $SESSION_NAME:$pane_info "source ~/.bashrc && cd $ROOT_DIR && export PYTHONPATH=$ROOT_DIR/src:${PYTHONPATH:-} && export GIT_PYTHON_REFRESH=quiet && export EXP_NAME=$EXP_NAME && \
            STATUS_FILE='$status_file' LOG_FILE='$log_file' RESULT_FILE='$result_file' && \
            MUJOCO_EGL_DEVICE_ID=0 CUDA_VISIBLE_DEVICES=0,1,2,3 $PYTHON_BIN experiments/libero/eval_libero_single.py \
            task=$CONFIG ckpt=$CKPT \
            EVALUATION.task_suite_name=$suite EVALUATION.task_id=$task_id EVALUATION.device=cuda:$gpu_id gpu_id=$gpu_id \
