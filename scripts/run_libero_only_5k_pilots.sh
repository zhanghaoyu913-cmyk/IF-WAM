...
RUN_TAG="${RUN_TAG:-libero_only_5k_seed${SEED}}"
...
LOG_ROOT="${LOG_ROOT:-${ROOT}/runs/libero_only_5k_pilots/logs}"
REPORT_ROOT="${REPORT_ROOT:-${ROOT}/reports/libero_only_5k_pilots}"
EVAL_ROOT="${EVAL_ROOT:-${ROOT}/evaluate_results/libero_only_5k_pilots}"
...
mkdir -p "${LOG_ROOT}" "${REPORT_ROOT}" "${EVAL_ROOT}"
cd "${ROOT}"
...
require_file() {
  local path="$1"
  if [[ ! -f "${path}" ]]; then
    echo "[libero-5k] missing required file: ${path}" >&2
    exit 1
  fi
}
...
require_dir() {
  local path="$1"
  if [[ ! -d "${path}" ]]; then
    echo "[libero-5k] missing required directory: ${path}" >&2
    exit 1
  fi
}
...
prepare_run_dir() {
  ...
  if [[ "${dir}" != "${ROOT}/runs/"* ]]; then
    echo "[libero-5k] refusing to clean unexpected run dir: ${dir}" >&2
    exit 1
  fi
  rm -rf "${dir}"
  mkdir -p "$(dirname "${dir}")"
...
train_stage() {
  ...
  echo "[libero-5k] train start label=${label} cfg=${cfg} steps=${max_steps} resume=${resume_path}"
  ...
  echo "[libero-5k] train done label=${label} final=${final_ckpt}"
}

context_usage() {
  local label="$1" cfg="$2" ckpt="$3" step="$4"
  require_file "${ckpt}"
  local out="${REPORT_ROOT}/context_usage_${label}_${step}"
  echo "[libero-5k] context usage label=${label} step=${step} ckpt=${ckpt} out=${out}"
  CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES%%,*}" "${PYTHON_BIN}" tools/diagnose_gridflow_context_usage.py \
    --config "${cfg}" \
    --checkpoint "${ckpt}" \
    --num-batches 2 \
    --output-dir "${out}" \
    ...
    --precision bf16 \
    --seed "${SEED}" \
    2>&1 | tee "${LOG_ROOT}/context_usage_${label}_${step}.log"
}
...
raw_grounding() {
  ...
  train_stage "${label}_raw_grounding" "ifwam_libero_gridfm_raw_grounding_1epoch" "${RAW_MAX_STEPS}" "${pilot_ckpt}" "libero-only-5k-raw-grounding"
}
...
screening_eval() {
  ...
  echo "[libero-5k] screening eval label=${label} ckpt=${ckpt} output=${output_dir}"
  ...
  echo "[libero-5k] eval done label=${label} summary=${output_dir}/summary.csv"
}
...
declare -A CFGS=(
  [P0]="libero_p0_nogrid_continued_5k"
  [P1]="gridfm_one_way_direction_presence_loss_only"
  [P2]="grid_aux_learned_query_direction_presence"
)

for label in P0 P1 P2; do
  train_stage "${label}" "${CFGS[$label]}" "${PILOT_STEPS}" "${BASE_CKPT}" "libero-only-5k-pilots"
  if [[ "${label}" != "P0" ]]; then
    run_id="${RUN_TAG}_${label}"
    run_dir="$(run_dir_for "${CFGS[$label]}" "${run_id}")"
    context_usage "${label}" "${CFGS[$label]}" "${run_dir}/checkpoints/weights/$(step_tag 1000)" "step001000"
    context_usage "${label}" "${CFGS[$label]}" "${run_dir}/checkpoints/weights/$(step_tag 5000)" "step005000"
  fi
done
...
echo "[libero-5k] complete RUN_TAG=${RUN_TAG}"
echo "[libero-5k] reports: ${REPORT_ROOT}"
echo "[libero-5k] eval: ${EVAL_ROOT}"
